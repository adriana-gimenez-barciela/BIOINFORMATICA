...
datos <- read.csv("/Users/adrigimenez/Desktop/datos_biomed.csv", stringsAsFactors = FALSE)
...
head(datos)
# realizo resumen estadístico general
summary(datos)
# obtenemos dimensiones del data frame (número de filas y columnas)
dim(datos)
# para estructura del conjunto de datos
str(datos)
# Contamos cuántas variables hay
num_variables <- ncol(datos)
cat("Número de variables:", num_variables, "\n")
# Contamos cuántos tratamientos distintos hay
...
# vamos a obtener etiquetas combinadas para el eje X
...
# necesitamos función para dibujar un violín
...
# vamos a preparar gráfico vacío
...
# vamos a asignar colores según el tratamiento
...
# vamos a obtener el gráfico de dispersión
...
tratamientos <- unique(datos$Tratamiento)
# tenemos que configurar la ventana gráfica con una fila por cada tratamiento
...
# vamos a configurar ventana gráfica: 1 fila y 3 columnas
par(mfrow = c(1, 3))
...
# tenemos que calcular la media de glucosa por tratamiento
media_glucosa <- aggregate(Glucosa ~ Tratamiento, data = datos, FUN = mean)
# vamos a calcular la desviación estándar de glucosa por tratamiento
...
# Extraemos los datos según el tratamiento
...
# Si tenemos datos son normales → usar ANOVA
cat("\nANOVA: Glucosa ~ Tratamiento\n")
anova_model <- aov(Glucosa ~ Tratamiento, data = datos)
print(summary(anova_model))
# Si tenemos datos NO son normales → usar Kruskal-Wallis
